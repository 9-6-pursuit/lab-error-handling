...
function filterProductsByPriceRange(products, min, max) {
  ...
  if (products.length === 0) {
    throw "Products array cannot be empty.";
  }

  if (typeof min !== "number" || typeof max !== "number") {
    throw "Minimum and maximum prices must be numbers.";
  }


  if (max === 0) {
    throw "Maximum price cannot be 0.";
  }

  if (min > max) {
    throw "Minimum price cannot be greater than maximum price.";
  }

  if (min < 0 || max < 0) {
    throw "Prices cannot be negative.";
  }
  ...
  for (let product of products) {
    ...
    if (!product.priceInCents) {
      throw "One or more products is missing a price.";
   }
