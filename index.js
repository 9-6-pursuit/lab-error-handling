...
function getCartTotal(cart) {
  if (!cart.length) {
    ...
    // throw stops running the code if there is an error. 
  }
  let result = 0;
...
function filterProductsByPriceRange(products, min, max) {
  ...
  if (min < 0 || max <= 0) {
    throw 'Min must be greater than or equal to 0, and max must be greater than 0'
  }
...
*/
function getTotalOfAllProductsByPriceRange(products, min, max) { //throw catch watches for errors. 
  try { // for try you add the function you want to run if there were no errors. 
